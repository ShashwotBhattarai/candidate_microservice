...
} from "../services/saveUserDetailsToDatabase.service";
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const findSavedS3keyResponse = await findSavedS3key(
      req.headers.authorization || ""
    );
    ...
    if (status3 && status2 == 200) {
      const deleteFileResponse = await new S3DeleteService().deleteFileFromS3(
        oldKey
      );
