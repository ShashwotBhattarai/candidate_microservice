...
import { uploadFileToS3BadBucket } from "../services/s3-upload-bad-bucket.service";
...
describe("S3 file upload service", () => {
  ...
  test("file gets uploaded to s3 bad bucket", async () => {
    ...
    const result = await uploadFileToS3BadBucket(
      mockBuffer,
      "application/pdf",
      "sfsf",
    );
    ...
    expect(result.message).toBe("new file uploaded to s3 bad bucket");
  });
  ...
  test("s3 error", async () => {
    ...
    try {
      await uploadFileToS3BadBucket(mockBuffer, "application/pdf", "sfsf");
    } catch (error) {
      expect(error).toEqual(new Error("error in uploadFileToS3 badbucket"));
    }
  });
});
