...
const sendMessageToQueueSpy = jest.spyOn(SQSService.prototype, "sendMessageToQueue");
...
describe("uploadCandidateInfo Service", () => {
	test("error if file is missing", async () => {
		const finalResult = await uploadCandidateInfoService(accessTokenMock, mockNullFile, bodyMock);
